...
public final class zzto {
    ...
    public static FirebaseException zzb(Status status, AuthCredential authCredential, String str, String str2) {
        int V = status.V();
        if (V != 17012 && V != 17007 && V != 17025) {
            return zza(status);
        }
        FirebaseAuthUserCollisionException firebaseAuthUserCollisionException = new FirebaseAuthUserCollisionException(zzd(V), zze(zzf(V), status));
    ...
    private static String zze(String str, Status status) {
        if (TextUtils.isEmpty(status.W())) {
            return str;
        }
        return String.format(String.valueOf(str).concat(" [ %s ]"), new Object[]{status.W()});
    }
